...
def _pair_by_scene_rule(in_dir: Path, gt_dir: Path) -> List[Tuple[str, str]]:
    """네가 준 scene 규칙(DGT vs D##, scene_key 동일)으로 1:N 매칭"""
    pairs = []
    gt_map: Dict[str, str] = {}
...
def _build_pairs(root: Path, split: str, tasks: List[str]) -> List[Tuple[str, str, str]]:
    """dataset/{dust,fog,rain}/{train,val,test}/{input,gt}/* 를 스캔해 (inp, gt, task) 리스트 생성"""
    out = []
    for task in tasks:
...
class AllWeatherDatasetFT(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.parse_patches:
            ...
            # whole-image: 리사이즈를 16의 배수로 맞춤(원 코드 호환)
...
class AllWeather:
    """
    원래 AllWeather 래퍼와 동일한 인터페이스(get_loaders) 유지.
    ...
    def get_loaders(self, parse_patches=True, validation='snow'):
        # 우리 데이터: train/val/test 모두 같은 폴더 규칙
        data_dir = self.config.data.data_dir
        img_size = self.config.data.image_size
        tasks = getattr(self.config.data, "tasks", ["dust", "fog", "rain"])

        train_ds = AllWeatherDatasetFT(data_dir, split="train", image_size=img_size,
                                       n_patches=self.config.training.patch_n,
                                       parse_patches=parse_patches, tasks=tasks)
        # validation 인자는 무시하고 val split을 사용(우리 데이터 기준)
        val_ds   = AllWeatherDatasetFT(data_dir, split="val", image_size=img_size,
                                       n_patches=self.config.training.patch_n,
                                       parse_patches=parse_patches, tasks=tasks)
        ...
        train_loader = DataLoader(train_ds, batch_size=self.config.training.batch_size,
                                  shuffle=True, num_workers=self.config.data.num_workers, pin_memory=True)
        val_loader   = DataLoader(val_ds, batch_size=self.config.sampling.batch_size,
